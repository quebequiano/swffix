/*	SWFFix 0.2 <http://www.swffix.org>
...
var SWFFix = function() {
	...
	var ua = function() {
		...
		if (navigator.plugins && typeof navigator.plugins["Shockwave Flash"] == "object") {
			...
			var fp6Crash = false;
			try { var a = new ActiveXObject("ShockwaveFlash.ShockwaveFlash.7"); }
			catch(e) {
				try { 
	...
	function registerObject(objectIdStr, configObj) {
		if (!ua.w3cdom) { return; }
		var regObj = {};
		regObj.id = objectIdStr;
		createCSS("#" + objectIdStr, "visibility:hidden");
		if (configObj.swfversion) {
			var v = configObj.swfversion.split(".");
			regObj.swfVersion = [parseInt(v[0], 10), parseInt(v[1], 10), parseInt(v[2], 10)];
		}
		else {
			regObj.swfVersion = [0,0,0];
		}
		regObj.expressInstall = configObj.expressinstall ? configObj.expressinstall : false;
